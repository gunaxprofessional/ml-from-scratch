...
class LogisticRegressionRidge:
    ...
    def fit(self, X, y):
        ...
        for i in range(self.num_iterations):
            ...
            dw = (1 / n_samples) * np.dot(X.T, (y_pred - y)) + \
                2 * self.lambda_reg * self.weights  # L2 regularization term
    ...
    def compute_cost(self, y, y_pred):
        # Binary cross-entropy cost with Ridge (L2) regularization
        n_samples = y.shape[0]
        cost = -(1 / n_samples) * np.sum(y * np.log(y_pred + 1e-15) +
                                         (1 - y) * np.log(1 - y_pred + 1e-15))
        ridge_penalty = self.lambda_reg * \
            np.sum(self.weights**2)  # L2 regularization
        return cost + ridge_penalty
