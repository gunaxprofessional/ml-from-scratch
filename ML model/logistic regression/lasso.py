...
class LogisticRegressionLasso:
    def __init__(self, learning_rate=0.01, num_iterations=1000, lambda_reg=0.1):
        ...
        self.lambda_reg = lambda_reg  # Lasso regularization parameter
        self.weights = None
        self.bias = None
    ...
    def fit(self, X, y):
        ...
        for i in range(self.num_iterations):
            ...
            dw = (1 / n_samples) * np.dot(X.T, (y_pred - y)) + \
                self.lambda_reg * np.sign(self.weights)
    ...
    def compute_cost(self, y, y_pred):
        # Binary cross-entropy cost with Lasso (L1) regularization
        n_samples = y.shape[0]
        cost = -(1 / n_samples) * np.sum(y * np.log(y_pred + 1e-15) +
                                         (1 - y) * np.log(1 - y_pred + 1e-15))
        lasso_penalty = self.lambda_reg * \
            np.sum(np.abs(self.weights))  # L1 regularization
        return cost + lasso_penalty
...
# Train model with Lasso regularization
model = LogisticRegressionLasso(
